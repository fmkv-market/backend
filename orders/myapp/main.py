...
async def lifespan(app: FastAPI):
    await broker.start()
    await broker.declare_queue(RabbitQueue(name=QueueConfig.user_created, durable=True))
    await broker.declare_queue(RabbitQueue(name=QueueConfig.item_added, durable=True))
    await broker.declare_queue(RabbitQueue(name=QueueConfig.item_removed, durable=True))
    yield
    await broker.stop()
